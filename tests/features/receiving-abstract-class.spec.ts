...
abstract class SampleAbstractClass {
	abstract foo(value: number): Promise<string>;
	abstract boo(value: number): Promise<string>;
}

const makeSut = () => {
	const sut = makeClassLikeStub<SampleAbstractClass>('foo');

	return { sut };
};

describe('makeClassLikeStub receiving Abstract Class', () => {
	it('should mock foo', () => {
		const { sut } = makeSut();
